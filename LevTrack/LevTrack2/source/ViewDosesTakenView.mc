...
class ViewDosesTakenView extends WatchUi.View {
    ...
    function onShow() as Void {
        System.println("View Doses Taken screen shown");
    }
    ...
    function onUpdate(dc as Graphics.Dc) as Void {
        ...
        if (schedules.size() == 0) {
            ...
            // Collect all taken doses from all schedules
            var allDoses = [];
            ...
            for (var i = 0; i < schedules.size(); i++) {
                ...
                for (var j = 0; j < dosesTaken.size(); j++) {
                    allDoses.add({
                        "timestamp" => dosesTaken[j],
                        "scheduleIndex" => i,
    ...
    function onHide() as Void {
        System.println("View Doses Taken screen hidden");
    }
    ...
    function scrollDown() as Void {
        ...
        // Count total taken doses
        var totalDoses = 0;
        for (var i = 0; i < schedules.size(); i++) {
            var schedule = schedules[i] as Dictionary;
            var dosesTaken = schedule["dosesTaken"] as Array;
            totalDoses += dosesTaken.size();
        }
...
class ViewDosesTakenDelegate extends WatchUi.BehaviorDelegate {

    var _view as ViewDosesTakenView;

    function initialize(view as ViewDosesTakenView) {
